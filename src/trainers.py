...
class Trainer:
    ...
    def info_nce(self, z_i, z_j, temp, batch_size, sim='dot', intent_id=None, use_neg=True):
        """
        ...
        if sim == 'cos':
            ...
            # sim = torch.mm(z, z.t()) / (temp * math.sqrt(self.args.hidden_size))
        ...
        if self.args.f_neg and use_neg:
    ...
    def cl_loss(self, output, aug_output):
        logits, labels = self.info_nce(output[:, -1, :], aug_output[:, -1, :],
                                       self.args.temperature, output.shape[0], self.sim, use_neg=False)
    ...
    def iteration(self, epoch, dataloader, full_sort=True, train=True):
        ...
        if train:
            ...
            for i, (rec_batch) in rec_cf_data_iter:
                ...
                # ---------- prediction task -----------------------#
                logits, output, aug_output = self.model(input_ids)
                rec_loss = nn.CrossEntropyLoss()(logits, target_pos[:, -1])

                # ---------- contrastive learning task -------------#
                cl_loss = self.cl_loss(output, aug_output)

                # ---------- multi-task learning -------------------#
                joint_loss = self.args.rec_weight * rec_loss + self.args.cl_weight * cl_loss
            ...
            post_fix = {
                ...
                "cl_avg_loss": "{:.4f}".format(cl_avg_loss / len(rec_cf_data_iter)),
            ...
            if full_sort:
                ...
                for i, batch in rec_data_iter:
                    ...
                    rating_pred, _, _ = self.model(input_ids)  # [BxLxH]
