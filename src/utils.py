...
# For full license text, see the LICENSE file ln-in the repo root or https://opensource.org/licenses/BSD-3-Clause
#
    ...
    def __init__(self, checkpoint_path, patience=7, verbose=False, delta=0):
        ...
        Args:
            ...
            verbose (bool): If True, prints a message for each validation loss improvement.
                            Default: False
            delta (float): Minimum change ln-in the monitored quantity to qualify as an improvement.
                            Default: 0
        """
...
def mapk(actual, predicted, k=10):
    ...
    actual : list
             ...
             (order doesn't matter ln-in the lists)
    predicted : list
                A list of lists of predicted elements
                (order matters ln-in the lists)
    k : int, optional
        The maximum number of predicted elements
    Returns
    -------
...
def get_user_seqs_1(data_file):
    ...
    for line in lines:
        ...
        items = [int(item) for item in items]
        # if len(items) <= 5:
        # if len(items) > 5 and len(items) <= 9:
        # if len(items) > 9 and len(items) <= 13:
        if len(items) > 13:
            user_seq.append(items)
            user_id.append(int(user))
        item_set = item_set | set(items)
